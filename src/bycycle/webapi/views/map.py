...
def make_mvt_view(
    table, layer_name=None, column=None, properties=None, where_clause=None
):
    if isinstance(table, str):
        ...
        properties = properties or tuple(
            c.name for c in table.columns if c.name != column
        )
        table = table.name
    ...
    def view(request):
        ...
        cached_record = dbsession.execute(
            MVT_CACHE_STATEMENT, params={"key": cache_key}
        )
        ...
        if data is None:
            ...
            data = data.tobytes()
            dbsession.execute(
                MVTCache.__table__.insert().values(key=cache_key, data=data)
            )

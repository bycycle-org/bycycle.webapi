...
def deploy(
    ...
    prepare: Annotated[bool, arg(help="Run preparation tasks (local)")] = True,
    dijkstar: Annotated[bool, arg(help="Run Dijkstar tasks (remote)")] = True,
    deploy: Annotated[bool, arg(help="Run deployment tasks (remote)")] = True,
    # Corresponding tags will be skipped unless set.
    clean: Annotated[bool, arg(help="Remove local build directory")] = False,
    overwrite: Annotated[bool, arg(help="Remove remote build directory")] = False,
    tags: Annotated[
        tuple[str, ...], arg(help="Run *only* tasks corresponding to these tags")
    ] = (),
    skip_tags: Annotated[
        tuple[str, ...], arg(help="Skip tasks corresponding to these tags")
    ] = (),
    yes: Annotated[bool, arg(help="Deploy without confirmation")] = False,
    echo=True,
):
    ...
    ansible_args = get_ansible_args(
        env, version=version, tags=tags, skip_tags=skip_tags
    )
...
def get_ansible_args(
    env, *, root=None, tags=(), skip_tags=(), version=None, extra_vars=None
):
    ...
    return [
        ...
        paths["inventory"],
        "--extra-var",
        "root={root}".format(root=paths["root"]),
        "--extra-var",
        f"env={env}",
...
def nginx(cmd, raise_on_error=True):
    remote(
        ("systemctl", cmd, "nginx.service"), sudo=True, raise_on_error=raise_on_error
    )
...
def push_nginx_config(host):
    sync(
        "ansible/roles/provision/files/etc/nginx/",
        "/etc/nginx/",
        host,
        sudo=True,
        mode="u=rw,g=r,o=r",
...
def uwsgi(command, raise_on_error=True):
    remote(
        ("systemctl", command, "uwsgi.service"),
        sudo=True,
